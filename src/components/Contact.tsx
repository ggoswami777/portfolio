...
export const Contact = () => {
  ...
  return (
    <section
      id="contact"
      className="py-10 px-4 w-full relative overflow-hidden"
    >
      <div className="max-w-4xl mx-auto text-center flex flex-col items-center relative z-10">
        ...
        <div className="flex flex-row gap-4 md:gap-8 items-center justify-center mb-16 w-full max-w-md px-2">
          <ContactButton
            ...
            label={t("BookCall")}
            // Use your actual Cal.com username and event slug here
...
const ContactButton = ({ icon, label, calLink, onClick }: any) => (
  <div className="animated-border-container flex-1 min-w-[140px]">
    <button
      ...
      onClick={onClick}
      className="inner-content group relative w-full py-3.5 px-4 flex items-center justify-center gap-2 transition-all duration-500 hover:bg-white hover:shadow-[0_0_40px_rgba(255,255,255,0.4)]"
    >
      <span className="text-[var(--color-text-primary)] group-hover:text-black transition-colors duration-300">
        {icon}
      </span>
      <span className="text-[var(--color-text-primary)] group-hover:text-black font-bold text-[13px] md:text-sm whitespace-nowrap transition-colors duration-300">
        {label}
      </span>
    </button>
  </div>
